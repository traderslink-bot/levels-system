...
export function buildTradersLinkAiPriceActionPacket(
  ...
  if (!hasUsableTradersLinkAiPriceAction(context, dataAsOf)) {
    throw new Error("TradersLink AI Read requires recent full-session intraday price action.");
  }
  ...
  const sessionCoverage = [...new Set(
    annotated.map((item) => item.classified.session),
  )];
  const includesRegularHours = sessionCoverage.some((session) =>
    session === "opening_range" || session === "regular"
  );
  const includesPrePostMarket = sessionCoverage.some((session) =>
    session === "premarket" || session === "after_hours"
  );
  ...
  return {
    ...
    timeframe: "5m",
    sessionCoverage,
    includesRegularHours,
    includesPrePostMarket,
