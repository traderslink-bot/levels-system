...
const FAILURE_RECOVERY_SCHEMA = {
  ...
  properties: {
    ...
    setupRestorePrice: {
      type: "number",
      description: "Higher reclaim that restores the original bullish setup; it must be strictly above firstReclaimPrice and at or above cautionBelow when cautionBelow is available.",
    },
    firstObjectivePrice: {
      type: ["number", "null"],
...
function assertTradersLinkAiTradeMap(
  ...
  if (read.failureRecovery) {
    ...
    const minimumRestorePrice = read.cautionBelow.price ?? read.momentumFailure.price;
    if (minimumRestorePrice !== null && isBelow(recovery.setupRestorePrice, minimumRestorePrice)) {
      fail("failureRecovery setup restore price cannot remain below the published caution or failure boundary");
    }
    if (
