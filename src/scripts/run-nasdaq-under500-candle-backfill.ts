...
import {
  ...
  NASDAQ_UNDER500_DEFAULT_TIMEFRAMES,
  NASDAQ_UNDER500_LOOKBACKS,
  stageToBucket,
...
console.log(
  `[NasdaqUnder500CandleBackfill] mode=${execute ? "execute" : "dry_run"} stage=${stage ?? "all"} symbols=${plan.totals.symbols} selected=${plan.totals.selectedForFetch} covered=${plan.totals.covered} partial=${plan.totals.partial} missing=${plan.totals.missing}`,
);
