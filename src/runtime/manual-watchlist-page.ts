...
<head>
  ...
  <style>
    ...
    .adapter-output { white-space: pre-wrap; overflow-wrap: anywhere; background: #f7f9fc; border: 1px solid #dbe3ee; border-radius: 8px; padding: 12px; margin-top: 10px; font: 12px/1.45 ui-monospace, SFMono-Regular, Consolas, monospace; max-height: 520px; overflow: auto; }
...
<body>
  <main>
    ...
    <section>
      <h2>Deterministic Day Trade Adapter</h2>
      <div class="field-hint">Runtime-only test surface. It does not publish a website plan or disable AI Read.</div>
      <div class="inline-control toggle-control">
        <label class="toggle-switch">
          <input id="day-trade-adapter-toggle" type="checkbox" />
          <span class="toggle-slider"></span>
          <span id="day-trade-adapter-label">Disabled</span>
        </label>
        <input id="day-trade-adapter-symbol" maxlength="10" placeholder="Ticker" aria-label="Adapter ticker" />
        <button class="secondary" id="day-trade-adapter-refresh" type="button">Refresh Ticker</button>
      </div>
      <div class="inline-status" id="day-trade-adapter-status"></div>
      <div class="adapter-output" id="day-trade-adapter-output">Adapter is disabled.</div>
    </section>
  ...
  <script>
    ...
    const dayTradeAdapterLabelEl = document.getElementById("day-trade-adapter-label");
    const dayTradeAdapterSymbolEl = document.getElementById("day-trade-adapter-symbol");
    const dayTradeAdapterRefreshEl = document.getElementById("day-trade-adapter-refresh");
    const dayTradeAdapterStatusEl = document.getElementById("day-trade-adapter-status");
    const dayTradeAdapterOutputEl = document.getElementById("day-trade-adapter-output");
    ...
    function buildEntryMeta(entry) {
      ...
      if (entry.tradersLinkAiReadFailure) {
        ...
        failure.textContent =
          ...
          " Ã¢â‚¬â€ trigger: " + String(aiReadFailure.trigger || "unknown") +
    ...
    function renderAiReadDetail() {
      ...
      aiReadDetailTitleEl.textContent = row.symbol + " Ã¢â‚¬â€ " + auditStatusLabel(row.state);
    ...
    function renderAutoSelectorControl(status) {
      ...
      for (const decision of (selector.recentDecisions || [])) {
        ...
        const title = document.createElement("strong");
        title.textContent = decision.symbol + " Ã¢â‚¬â€ qualification " + decision.score + " Ã¢â‚¬â€ live rank " + decision.rankingScore + " Ã¢â‚¬â€ current slot " + decision.slotSurvivalScore + (
          decision.qualified
            ? decision.promotionReady === false
              ? " Ã¢â‚¬â€ qualifies, promotion held"
              : " Ã¢â‚¬â€ qualifies, promotion-ready"
            : decision.haltRetentionProtected
              ? " Ã¢â‚¬â€ confirmed halt, retention protected"
              : " Ã¢â‚¬â€ rejected"
        );
        const detail = document.createElement("div");
        detail.className = "activity-detail";
    ...
    function formatAdapterCluster(cluster) {
      ...
      const range = cluster.low === cluster.high
        ? String(cluster.price)
        : String(cluster.low) + "-" + String(cluster.high);
      return range + " | " + cluster.strength + " | evidence " + cluster.evidenceCount +
        (cluster.provisional ? " | provisional" : "");
    }

    function renderDayTradeAdapter(payload) {
      ...
      if (!results.length) {
        dayTradeAdapterOutputEl.textContent = payload.enabled
          ? "Enabled. Waiting for an active ticker refresh."
          : "Adapter is disabled.";
        return;
      }
      dayTradeAdapterOutputEl.textContent = results.map((result) => {
        const diagnostics = result.diagnostics || {};
        const candles = Array.isArray(diagnostics.candles) ? diagnostics.candles : [];
        const transitions = payload.transitions?.[result.symbol] || [];
        return [
          result.symbol + " | " + result.state + " | confidence " + result.confidence,
          result.summary,
          "Origin " + String(result.extension?.sessionOrigin ?? "missing") +
            " | session high " + String(result.extension?.sessionHigh ?? "missing") +
            " | extension " + String(result.extension?.extensionFromOriginPct ?? "missing") + "%",
          "Needs to hold: " + formatAdapterCluster(result.plan?.needsToHold),
          "Pullback: " + formatAdapterCluster(result.plan?.pullbackArea),
          "Must clear: " + formatAdapterCluster(result.plan?.mustClear),
          "Continuation: " + formatAdapterCluster(result.plan?.continuation),
          "Expansion: " + (result.plan?.expansionTargets || []).map(formatAdapterCluster).join("; "),
          "Invalidation: " + formatAdapterCluster(result.plan?.invalidation),
          "ATR: " + String(diagnostics.atr?.reliability || "missing") +
            " | value " + String(diagnostics.atr?.value ?? "missing") +
            " | completed candles " + String(diagnostics.atr?.completedCandleCount ?? 0) +
            (diagnostics.atr?.reason ? " | " + diagnostics.atr.reason : ""),
          "Levels: " + JSON.stringify(diagnostics.levelInventory || {}),
          "Candles: " + candles.map((item) =>
            item.timeframe + " " + item.candleCount + " bars, " +
            item.completedCandleCount + " complete, stale=" + item.stale +
            ", missing=" + item.missingIntervalCount +
            ", zero-volume=" + item.zeroVolumeCount
          ).join(" | "),
          "Flags: " + ((diagnostics.flags || []).join(", ") || "none"),
          "Transitions kept: " + transitions.length,
        ].join(String.fromCharCode(10));
      }).join(String.fromCharCode(10) + String.fromCharCode(10));
    }

    async function loadDayTradeAdapter() {
      const symbol = String(dayTradeAdapterSymbolEl.value || "").trim().toUpperCase();
      const payload = await fetchJson(
        "/api/runtime/day-trade-adapter" + (symbol ? "?symbol=" + encodeURIComponent(symbol) : "")
      );
      renderDayTradeAdapter(payload);
    }

    async function setDayTradeAdapterEnabled() {
      dayTradeAdapterToggleEl.disabled = true;
      try {
        const response = await fetch("/api/runtime/day-trade-adapter", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ enabled: dayTradeAdapterToggleEl.checked }),
        });
        const payload = await response.json();
        if (!response.ok) throw new Error(payload.error || "Adapter update failed.");
        renderDayTradeAdapter(payload);
      } finally {
        dayTradeAdapterToggleEl.disabled = false;
      }
    }

    async function refreshDayTradeAdapterTicker() {
      const symbol = String(dayTradeAdapterSymbolEl.value || "").trim().toUpperCase();
      if (!symbol) throw new Error("Enter a ticker for the adapter refresh.");
      const response = await fetch("/api/runtime/day-trade-adapter/refresh", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ symbol }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Adapter refresh failed.");
      renderDayTradeAdapter(payload.status);
    }
    ...
    dayTradeAdapterToggleEl.addEventListener("change", () => {
      void setDayTradeAdapterEnabled().catch((error) => setStatus(String(error), true));
    });
    dayTradeAdapterRefreshEl.addEventListener("click", () => {
      dayTradeAdapterRefreshEl.disabled = true;
      void refreshDayTradeAdapterTicker()
        .catch((error) => setStatus(String(error), true))
        .finally(() => {
          dayTradeAdapterRefreshEl.disabled = false;
        });
    });
    ...
    async function refreshDashboard() {
      ...
      try {
        ...
        await Promise.all([loadEntries(), loadRuntimeStatus(includeDetails), loadDayTradeAdapter()]);
      } catch (error) {
        setStatus(String(error), true);
      } finally {
