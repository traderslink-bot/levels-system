...
- What changed:
  ...
  - refined the Yahoo opener formatting so partial Yahoo responses only show fields that came back with usable data instead of printing large `n/a` blocks
- Why this matters:
...
- What changed:
  ...
  - softened snapshot and extension posts from dashboard labels like `LEVEL SNAPSHOT`, `CURRENT READ`, `KEY LEVELS`, `FULL LADDER`, `NEXT LEVELS`, `SIDE`, and `LEVELS` into trader-facing labels such as `level map`, `What price is doing now`, `Closest levels to watch`, `More support and resistance`, and `next levels to watch`
