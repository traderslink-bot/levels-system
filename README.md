...
> Deprecated local project: do not use this `C:\Users\jerac\Documents\TraderLink\levels-system` checkout for new work. Continue levels-system work in the v2/current worktree, currently `C:\Users\jerac\Documents\TraderLink\levels-system-post-mtf-handoff-stability`, unless the user explicitly provides a different v2 path.
...
- Runtime health now includes startup-cache proof: warming symbols, restored cache age, blocked cached snapshots, and the explicit rule that Discord startup snapshots require fresh candles.
- Startup cache coverage can be audited with `npm run startup:cache-readiness`; it shows which active symbols have enough cached daily/4h/5m candles for fast restore and repeats the safety rule that Discord snapshots still wait for fresh candle refresh.
...
- First snapshot trade maps now include a `Main trade area` line. When higher resistance is not available in the current snapshot, the map says a fresh higher-resistance check is needed before treating the path as open instead of implying no resistance exists.
- First snapshot level context now flags clustered nearby levels as practical zones, so low-priced names with several tight penny levels are framed around the zone/reaction instead of every exact print.
- Candle reaction context now uses the small-cap meaningful-move floor when classifying support losses, reclaims, and clean closes through resistance. Minor one-cent wiggles can remain `indecision` with `materialityLabel=minor` until the candle reaction is large enough to matter.
- Support/resistance level quality can be calibrated with `npm run levels:calibrate -- <session-folder-or-discord-delivery-audit.jsonl>`. The report rebuilds levels at saved post time, validates future 5-minute candle reactions, audits forward-ladder completeness without inventing levels, writes a gate artifact, and emits coverage/backfill hints when the audit is unproven.
...
- The saved-data replay simulator now infers practical zone/range-box context from older Discord audit rows that predate the newer metadata fields. This makes old CYCU-style chop logs test the current `alert_range_box_chop` / `alert_same_story_not_material` policy instead of overstating how much current code would repost.
...
- End-of-day symbol verdicts now include candle-backed evidence when cache is available: first-snapshot score, execution-relation replay counts, missing-forward-resistance counts, missed-move counts, volume may-help/hide counts, and representative evidence examples.
...
- Why-no-post replay proof can be generated with `npm run audit:why-no-post -- <session-folder-or-discord-delivery-audit.jsonl>`; it writes `why-no-post-replay-proof.json` / `.md` and classifies whether quiet behavior was supported by candles, preserved meaningful moves, may have hidden a move, or is unproven because candle evidence is missing. Reports now include concrete candle-backed move examples with nearest saved posts so `may_hide` findings are reviewable, not abstract. Single-session reports include balanced replay suppression evidence; `-- --all-sessions` scans all saved long-run sessions.
...
- The live post policy can now suppress repeated non-accepted flip chatter inside the same practical trade box with `practical_area_flip_chop`, while accepted breaks, critical changes, and true expansion remain eligible to post.
- If a runner clears the highest surfaced resistance and extension levels are unavailable, the runtime can refresh candles for higher resistance and retry the extension post rather than leaving the thread with a suspicious no-forward-level read.
...
- Shared-engine inventory, warehouse health, saved-session candle calibration, bulk import readiness, staged backfill priority, and dry-run warehouse backfill can be checked with `npm run engine:capabilities`, `npm run candles:audit -- data/candles`, `npm run candles:calibrate -- <session-folder>`, `npm run candles:import-readiness -- <session-folder>`, `npm run candles:backfill-priority -- <session-folder>`, `npm run candles:backfill-manifest -- --priority-report <priority-json> --stage 1`, and `npm run candles:backfill -- <session-folder> --max-tasks 8`.
...
- Bulk candle backfill plans now also report provider batches, coalesced trade-request counts, estimated candle counts, avoided task counts, and largest task size so IBKR or a future provider can be protected before bulk imports run.
- Candle import safety can be reviewed with `npm run candles:import-safety -- <session-folder-or-discord-delivery-audit.jsonl>`; it writes `candle-import-safety.json` / `.md` with provider-pressure verdicts, avoided provider requests, missing tasks, provider batch sizes, largest task estimates, and symbol/session coverage. Use `-- --all-sessions` for broad saved-data pressure.
- Candle backfill priority can be reviewed with `npm run candles:backfill-priority -- <session-folder-or-discord-delivery-audit.jsonl>`; it writes `candle-backfill-priority.json` / `.md` with `fetch_first`, `fetch_next`, and `fetch_later` tasks ranked by quiet-risk, post-noise, missing-candle proof, timeframe importance, and provider-safe stage limits. Use `-- --all-sessions` before broad provider work so the first real fetches fill the evidence gaps most likely to affect trader reads.
- A safe Stage 1 provider handoff can be generated with `npm run candles:backfill-manifest -- --priority-report artifacts/candle-backfill-priority/candle-backfill-priority.json --stage 1`. It writes a manifest plus the exact dry-run command. The dry-run path can then be executed with `npm run candles:backfill -- --priority-report <priority-json> --priority-stage 1`; this recalculates current warehouse gaps and fetches nothing unless `--execute` is explicitly added.
...
- Dynamic/opening-range calibration can be generated with `npm run candles:dynamic-calibrate -- <session-folder-or-discord-delivery-audit.jsonl>`; it writes `dynamic-reference-calibration-report.json` / `.md` plus a generated trust gate and proves whether cached candles support opening-range, VWAP, EMA9, and EMA20 facts near saved posts. Use `-- --all-sessions` for broad saved-data trust review.
- Provider comparison readiness can be generated with `npm run candles:provider-compare -- --primary ibkr --comparison stub`; it compares cached provider candles for coverage, close drift, VWAP/EMA drift, and support/resistance count drift before any future provider switch is trusted.
- Provider comparison readiness now also checks cached average-volume drift and 5m market-structure drift, so provider swaps can be reviewed for volume/structure differences before runtime behavior trusts them.
- Market-structure calibration can be generated with `npm run structure:calibrate`; it joins saved 5m replay evidence with Discord alignment evidence and identifies symbols where stable structure is trusted for suppression versus still operator-only or chop-watch.
- Advanced candle context can be reviewed with `npm run candles:advanced-context`; it is operator-only and summarizes support/resistance counts, reference levels, gaps, VWAP/EMA availability, market structure, candle reaction, opening range, halt awareness, data quality, and first-post-plan lines.
- Advanced candle context now includes data-quality score, reasons, primary cause, and missing facts so weak reads can be traced to candle coverage, level ladder, liquidity context, halt/stale candles, or mixed causes.
- `npm run replay:monday` now includes first-snapshot audit, end-of-day verdicts, market-structure calibration, advanced candle context, and provider comparison readiness so closed-market review is less likely to skip the important candle-intelligence checks.
- End-of-day symbol verdicts now include first-map checklist failures, market-structure calibration, advanced-context status, and provider-readiness warnings in their candle-evidence section.
- Candle-intelligence regression gates now track first-map failures, structure-chop watch cases, advanced-context gaps, provider-readiness warnings, quiet periods that may hide candle-backed moves, and post-budget noise watch cases in addition to weak snapshots, missed moves, relation gaps, and missing forward resistance.
...
- Candle-intelligence regression gates can be generated with `npm run candles:regression-gate -- <session-folder-or-discord-delivery-audit.jsonl>`; it evaluates the regression pack as `pass`, `review`, or `fail` using explicit thresholds for major candidates, weak first maps, missing forward resistance, and missing candle evidence. Use `--preset strict`, `--preset review`, or `--preset exploratory` depending on whether the goal is release gating or broad evidence gathering.
...
- `npm run candles:backfill-priority -- <session-folder-or-discord-audit.jsonl>`
- `npm run candles:backfill-manifest -- --priority-report <priority-json> --stage 1`
- `npm run candles:backfill -- <session-folder-or-discord-audit.jsonl> --max-tasks 8`
- `npm run candles:provider-compare -- --primary ibkr --comparison stub`
