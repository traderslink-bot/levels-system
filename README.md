...
- To prototype the planned pre-level thread opener separately from the runtime, set `FINNHUB_API_KEY` in `.env` and run `npm run finnhub:test -- AAPL` to print a Finnhub-backed stock context preview in the terminal first.
...
- Identical `NEXT LEVELS` payloads are now also suppressed until the extension payload actually changes, so a symbol like `AMST` should not keep reposting the same resistance ladder every few minutes with no new information.
